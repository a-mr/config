...
do
	...
	case "$i" in
		d)
		    echo -ne "$src_cur"|grep \:|grep -v enclosure|awk {'printf ("%s\t%s %s\t", $2, $3, $4)'}|sed s/Aug/Август/g|sed s/,//g
			;;

		t)
			echo "$info_cur"|awk -F "температура " {'print $2'}|awk {'printf "t=" $1 "°C\t"'}
			;;
		p)
			echo "$info_cur"|awk -F "давление" {'print $2'}|awk {'printf "P=" $1 "мм.рт.ст.\t"'}
			;;
		w)
			echo "$info_cur"|awk -F "ветер" {'printf "Ветер:"  $2 "\t"'}
			;;
		*)
